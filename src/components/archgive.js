...
import { Calendar, Flag, MapPin, Activity, ChevronDown, ChevronLeft, ChevronRight } from "lucide-react";
...
const OlympicsScheduleApp = () => {
      ...
      async (resetPage = false) => {
        ...
        try {
          ...
          setSchedule((prevSchedule) =>
            resetPage ? data.events : [...prevSchedule, ...data.events]
          );
    ...
    useEffect(() => {
      if (country || selectedSport || selectedDate) {
        fetchSchedule(true);
      }
  ...
  const convertTime = (parisTime, targetLocation) => {
    try {
      ...
      const offsetDiff =
        TIME_ZONE_OFFSETS[targetLocation] - TIME_ZONE_OFFSETS["France"];
